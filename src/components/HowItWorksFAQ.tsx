...
export const HowItWorksFAQ = () => {
  ...
  return (
    <section id="faq" className="relative w-full max-w-[1280px] mx-auto px-6 lg:px-8 py-16">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
        
        {/* Left Column: How It Works */}
        <div>
          <h3 className="text-[#8E97B7] text-[12px] font-bold tracking-[0.18em] uppercase mb-8">
            How It Works
          </h3>
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4">
            {steps.map((step, idx) => (
              <motion.div 
                ...
                className="flex-1 flex flex-col items-center text-center p-6 rounded-[20px] bg-[#0c0f1c]/40 border border-divider relative group"
              >
                <div className="w-12 h-12 rounded-full bg-card-hover border border-divider flex items-center justify-center mb-4 relative shadow-inner">
                  {step.icon}
                  <div className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-primary flex items-center justify-center text-[10px] font-bold text-primary-text shadow-lg">
                    {step.num}
                  </div>
                </div>
                <h4 className="text-primary-text font-medium text-[17px] mb-2">{step.title}</h4>
                <p className="text-secondary-text text-[14px] leading-relaxed">{step.desc}</p>
        ...
        <div>
          ...
          <div className="flex flex-col gap-3">
            {/* Split into two columns within the grid if we want, but the reference shows 2 columns of FAQs */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
               {faqs.map((faq, idx) => (
                 <div 
                   key={idx}
                   onClick={() => setOpenFaq(openFaq === idx ? null : idx)}
                   className="rounded-xl bg-[#0c0f1c]/40 border border-divider cursor-pointer hover:bg-white/[0.03] transition-colors group overflow-hidden"
                 >
                   <div className="p-4 flex items-center justify-between">
                     <span className="text-primary-text text-[15px] font-medium pr-4">{faq.question}</span>
                     <ChevronDown className={`w-4 h-4 text-secondary-text transition-transform duration-300 ${openFaq === idx ? 'rotate-180' : ''}`} />
                   </div>
                   <div 
                     className={`overflow-hidden transition-all duration-300 ${openFaq === idx ? 'max-h-40 opacity-100 mb-4' : 'max-h-0 opacity-0'}`}
                   >
                     <p className="px-4 text-secondary-text text-[14px] leading-relaxed">
                       {faq.answer}
                     </p>
                   </div>
                 </div>
            ...
            <div className="mt-4 text-center sm:text-right">
              <a href="#faq" className="inline-flex items-center gap-2 text-primary hover:text-primary/80 text-sm font-medium transition-colors">
